...
#         medianIndex = (longestBlock.numPoints + 1) / 2
        medianIndex = int((longestBlock.numPoints + 1) / 2)

        idx = longestBlock.longestSideIndex()
#         longestBlock.points = sorted(longestBlock.points, cmp=lambda x,y: x[idx] - y[idx])
        longestBlock.points = sorted(longestBlock.points, key=cmp_to_key(lambda x,y: x[idx] - y[idx]))        
...
def main(path_img, n_cluster):
    img = Image.open(path_img)
    points = list(img.getdata())

    return median_cut(points, n_cluster)
